...
def degrees_through_years():
    ...
    keys = ["Licencjackie / inżynierskie", "Brak", "Magisterskie", "Doktorskie", "Podstawowe", "Profesorskie (lub dr hab.)", "Średnie", "Nieukończone studia", "Inne"]
    df = pd.DataFrame()
    for i in range(9):
      df[keys[i]] = data[i]
